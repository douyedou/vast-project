...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!documentId) return NextResponse.json(error("documentId 不能为空", 400))
    ...
    return NextResponse.json(error(`获取版本历史失败: ${sanitizeM06Text(err.message)}`, 500))
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const docWithCase = await getDocumentWithCase(documentId)
    ...
    return NextResponse.json(error(`恢复版本失败: ${sanitizeM06Text(err.message)}`, 500))
  }
}
